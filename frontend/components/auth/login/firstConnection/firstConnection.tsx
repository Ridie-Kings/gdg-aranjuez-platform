...
export default function FirstConnection() {
  ...
  return (
    <motion.div
      ...
      className="w-full flex flex-col items-center p-8 bg-background text-default"
    >
      {/* Recuadro detrás del formulario */}
      <div className="w-2/3 p-8 bg-background/80 backdrop-blur-md border border-customOrange rounded-lg shadow-lg">
        
        {/* Current Skill Level */}
        <div className="mb-8">
          <h2 className="text-xl font-bold mb-2 text-default">Nivel de Habilidad Actual</h2>
        ...
        {/* Career Stage */}
        <div className="mb-8">
          <h2 className="text-xl font-bold mb-2 text-default">Etapa de Carrera</h2>
          <div className="flex flex-col gap-2">
            <label className="flex items-center gap-2 text-customGray">
              <input
                type="radio"
            ...
            <label className="flex items-center gap-2 text-customGray">
              <input
                ...
                className="text-customOrange"
              />{" "}
              Profesional – Actualmente trabajo de manera profesional como ingeniero de software o gerente.
            </label>
          </div>
        </div>
        ...
        <div className="mb-8">
  <h2 className="text-xl font-bold mb-4 text-default">Lenguaje de Programación Base</h2>
  <div className="relative">
    ...
    >
      ...
      <option value="C++">C++</option>
       ...
       {/* Career Options */}
<div className="mb-8">
  <h2 className="text-xl font-bold mb-4 text-default">Opciones de Carrera</h2>
  <div className="flex flex-col gap-4">
    {careerOptions.map((option) => (
      <button
        key={option.id}
        className="flex items-center justify-start p-4 border-2 rounded-md bg-gray-800/60 backdrop-blur-md text-white hover:border-customOrange transition-colors duration-300 hover:bg-gray-700 shadow-lg hover:shadow-[0px_0px_15px_5px_rgba(255,117,24,0.5)]"
      >
        <img src={option.icon} alt={option.name} className="w-6 h-6 mr-4" />
        ...
        <button className="px-6 py-2 bg-customOrange hover:bg-customOrange rounded-md w-full text-white font-semibold">
           <p><Link href={"/home"} className="text-black">Guardar Preferencias</Link></p>
        </button>
      </div>
    </motion.div>
  );
